...
public class Lab_8 {
	...
	static public void badacz(int dl, int ilosc) throws UnirestException, IOException {
		...
		HttpResponse<String> calosc = Unirest.post("https://community-sentiment.p.mashape.com/text/")
				...
				.field("txt", PageStartAll).asString();
		...
		for (String str : words) {
			...
			if (str.length() == dl) {
				HttpResponse<String> response = Unirest.post("https://community-sentiment.p.mashape.com/text/")
						...
						.header("Accept", "application/json").field("txt", str).asString();
	...
	static void sort(String[] words, double[] values) {
		...
		int wczesniejsza = values.length - 1; // indeks pierwszej porownywanej
												// liczby; wartosc poczatkowa
		int pozniejsza = wczesniejsza - 1; // indeks drugiej porownywanej
											// liczby; wartosc poczatkowa

		double tmp;
		String tmp2;
		int tmp3;
		while (pozniejsza >= 0) // indeks drugiej porownywanej liczby musi byc
								// nie mniejszy niz zero
		{
			if (values[pozniejsza] > values[wczesniejsza]) // jesli druga liczba
															// jest wieksza niz
															// pierwsza
			{
			...
			} else // jesli druga liczba jest mniejsza lub rowna pierwszej,
